...
bool_t sal_call_holdon(SalOp *h,SalHoldStatus holdon)
{
	int err = 0;
	char *sdpmesg=NULL;
	osip_message_t *msg=NULL;
	SalMediaDescription *mediadesc=NULL;

	mediadesc = h->result;
	err=eXosip_call_build_request(h->did,"INVITE",&msg);
	if(err == 0 && mediadesc!=NULL){
		if (holdon == SalHold){
			mediadesc->holdon = SalHold;
			set_sdp_from_desc(msg,mediadesc);
		}else if (holdon == SalUnHold){
			mediadesc->holdon = SalUnHold;
			set_sdp_from_desc(msg,mediadesc);
		}
...
static bool_t call_failure(Sal *sal, eXosip_event_t *ev){
	...
	switch(code)
	{
		...
		break;
		case 491:
			ms_message("Response code 491");
			error=SalErrorFailure;
			sr=SqlReasonRequestPending;
			break;
		case 600:
			error=SalErrorFailure;
...
static void registration_success(Sal *sal, eXosip_event_t *ev){
	...
	}else if(h!=NULL && atoi(h->hvalue)==0) /*expires = 0 , ÔòÎª×¢Ïú*/
	{
		registered=FALSE;
		sal->callbacks.register_success(op,registered);
	}
...
static void other_request_reply(Sal *sal,eXosip_event_t *ev){
	...
	if (op==NULL){
		...
		{
			LinphoneCore *lc = sal_get_user_pointer(sal);
			int quality = ms_get_current_ms_time(NULL) - lc->option_time_start;
				if(lc->vtable.network_quality_monitor!=NULL)
					lc->vtable.network_quality_monitor(lc,quality);
			ms_message("network quality %d ms !",quality);
		}else
