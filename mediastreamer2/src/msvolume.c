...
//use our builtin agc
#if defined(HAVE_SPEEXDSP) && !defined(USE_BUILTIN_AGC)
...
static int volume_set_nr(MSFilter *f, void *arg){
	Volume *v=(Volume*)f->data;
	v->nr_enabled=*(int*)arg;
	return 0;
}
...
static void update_energy(int16_t *signal, int numsamples, Volume *v) {
	...
	en = (sqrt((float)(acc / numsamples))+1) / max_e;
...
static void volume_preprocess(MSFilter *f){
	...
	if (v->agc_enabled || v->nr_enabled){
		...
		if (v->speex_pp==NULL){
			...
			{
				tmp = 1;
				if (speex_preprocess_ctl(v->speex_pp,SPEEX_PREPROCESS_SET_NOISE_SUPPRESS,&tmp)==-1){
					ms_warning("Speex NR is not available.");
				}
			}
		}
#endif
...
static MSFilterMethod methods[]={
	...
	{	MS_VOLUME_ENABLE_NR	,	volume_set_nr		},
