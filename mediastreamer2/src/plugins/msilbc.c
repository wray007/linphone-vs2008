...
static void dec_process(MSFilter *f){
	...
	while ((im=ms_queue_get(f->inputs[0]))!=NULL){
		...
		if (nbytes%38==0 && s->nbytes!=NO_OF_BYTES_20MS)
		{
			...
			s->nbytes=NO_OF_BYTES_20MS;
			s->nsamples=BLOCKL_20MS;
			initDecode(&s->ilbc_dec,s->ms_per_frame,1);
		}
		...
		if (s->nbytes>0 && nbytes>=s->nbytes){
			...
			for (k=0;k<frame_per_packet;k++)
			{
				...
				iLBC_decode(samples,(uint8_t*)im->b_rptr+(k*s->nbytes),&s->ilbc_dec,good_packet);
...
MS_FILTER_DESC_EXPORT(ms_ilbc_enc_desc)
MS_FILTER_DESC_EXPORT(ms_ilbc_dec_desc)
