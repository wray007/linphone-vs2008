...
static void call_logs_write_to_config_file(LinphoneCore *lc){
	...
	for(i=0,elem=lc->call_logs;elem!=NULL;elem=elem->next,++i){
		...
		lp_config_set_string(cfg,logsection,"from",tmp);
		ms_free(tmp);
		lp_config_set_string(cfg,logsection,"caller",cl->caller);
		lp_config_set_string(cfg,logsection,"called",cl->called);
		lp_config_set_string(cfg,logsection,"record_file",cl->record_file);
...
static void call_logs_read_from_config_file(LinphoneCore *lc){
	...
	for(i=0;;++i){
		...
		if (lp_config_has_section(cfg,logsection)){
			...
			tmp=lp_config_get_string(cfg,logsection,"caller",NULL);
			if (tmp) strncpy(cl->caller,tmp,sizeof(cl->caller));
			tmp=lp_config_get_string(cfg,logsection,"called",NULL);
			if (tmp) strncpy(cl->called,tmp,sizeof(cl->called));
			tmp=lp_config_get_string(cfg,logsection,"record_file",NULL);
			if (tmp) strncpy(cl->record_file,tmp,sizeof(cl->record_file));
...
net_config_read (LinphoneCore *lc)
{
	...
	tmpstr=lp_config_get_string(lc->config,"net","crypt_key",NULL);
	lc->net_conf.crypt_key=tmpstr;
	linphone_core_set_transports_crypt_key(lc,tmpstr);
	tmp=lp_config_get_int(lc->config,"net","crypt_signal",SIGNAL_NONE);
	tmp1=lp_config_get_int(lc->config,"net","crypt",PHONE_CRYPT_NONE);
	linphone_core_set_transports_crypt(lc,tmp,tmp1);
...
*/
	...
	linphone_core_enable_nr(lc,
		lp_config_get_int(lc->config,"sound","nr",0));

	linphone_core_enable_record(lc,
		lp_config_get_int(lc->config,"sound","record",0));
...
static void sip_config_read(LinphoneCore *lc)
{
	...
	if (lp_config_get_int(lc->config,"sip","sip_random_port",0)) {
		...
		tr.udp_port=lp_config_get_int(lc->config,"sip","sip_port",8060);
	}
    ...
    const char *factory_config_path, void * userdata)
{
	...
	linphone_core_assign_payload_type(&payload_type_g7231,4,NULL);
	...
	linphone_core_assign_payload_type(&payload_type_g726_32,19,NULL);
	linphone_core_assign_payload_type(&payload_type_bv16,106,NULL);
	linphone_core_assign_payload_type(&payload_type_bv32,107,NULL);
...
int linphone_core_network_get_idel_udp_port(int sport)
{
	...
	while(success == FALSE)
	{
		...
		if(bind(sock,(struct sockaddr *)&name ,sizeof(name))== 0 )
		{
			free_port = p;
			success = TRUE;
			if(sport != free_port) ms_error("UDP port %i seems already in use ! try use next port %d.",p-1,free_port);
		}
		p += 1;
	}
#ifdef WIN32
...
{
	return linphone_core_network_get_idel_udp_port(lc->rtp_conf.audio_rtp_port);
}
...
int linphone_core_get_video_port(const LinphoneCore *lc){
	return linphone_core_network_get_idel_udp_port(lc->rtp_conf.video_rtp_port);
}
...
static char _ua_name[64]=LIBSIPUA3_NAME;
...
/*	snprintf(ua_string,sizeof(ua_string)-1,"%s/%s (eXosip2/%s)",_ua_name,_ua_version,
	...
	snprintf(ua_string,sizeof(ua_string)-1,"%s/%s",_ua_name,_ua_version
		);
...
static int apply_transports(LinphoneCore *lc){
	...
	if (tr->udp_port>0){
		tr->udp_port = linphone_core_network_get_idel_udp_port(tr->udp_port);
...
void linphone_core_iterate(LinphoneCore *lc){
	...
	if (lc->call!=NULL){
		...
		if (call->dir==LinphoneCallIncoming && call->state==LCStateRinging){
			...
			if(lc->audiostream!=NULL) audio_stream_iterate(lc->audiostream);
#ifdef VIDEO_ENABLED
			if (lc->videostream!=NULL)
...
int linphone_core_call_start_recoding(LinphoneCore *lc, LinphoneCall *call,const char *path)
{
	...
	if (!call || call->state != LCStateAVRunning) -1;
	...
	if(lc->audiostream){
		...
		if(audio_stream_recored_file_enabled(lc->audiostream))
			audio_stream_record_start(lc->audiostream,record_path);
		else{
			linphone_core_enable_record(lc,TRUE);
...
int linphone_core_call_holdon(LinphoneCore *lc, LinphoneCall *call,bool_t holdon)
{
	if(!call) return;

	if(holdon){
		if(sal_call_holdon(call->op,SalHold)){
			if(lc->vtable.display_status) lc->vtable.display_status(lc,_("Holding"));
			linphone_core_stop_media_streams(lc, call);
		}
	}else{
		if(sal_call_holdon(call->op,SalUnHold)){
			if(lc->vtable.display_status) lc->vtable.display_status(lc,_("UnHold"));

			if(lc->audiostream!=NULL) return;
			linphone_core_init_media_streams(lc, call);
			linphone_core_start_media_streams(lc,call);
		}
	}
...
void linphone_core_init_media_streams(LinphoneCore *lc, LinphoneCall *call){
	...
	audio_stream_enable_noise_reduction(lc->audiostream,linphone_core_nr_enabled(lc));
	audio_stream_enable_recored_file(lc->audiostream,linphone_core_record_enabled(lc));
	audio_stream_set_quality_callback(lc->audiostream,lc,lc->vtable.media_quality_monitor);
	{
...
void linphone_core_enable_nr(LinphoneCore *lc, bool_t val){
	lc->sound_conf.nr=val;
}

bool_t linphone_core_nr_enabled(const LinphoneCore *lc){
	return lc->sound_conf.nr;
}

bool_t linphone_core_record_enabled(const LinphoneCore *lc){
	return lc->sound_conf.record;
}

void linphone_core_enable_record(LinphoneCore *lc, bool_t val){
	lc->sound_conf.record=val;
}
...
void net_config_uninit(LinphoneCore *lc)
{
	...
	lp_config_set_int(lc->config,"net","crypt",config->crypt);
	lp_config_set_int(lc->config,"net","crypt_signal",config->crypt_signal);
	if (config->crypt_key!=NULL){
		lp_config_set_string(lc->config,"net","crypt_key",config->crypt_key);
		ms_free(lc->net_conf.crypt_key);
	}
...
*/
void linphone_core_set_transports_crypt(LinphoneCore *lc,LCCryptSignal signal, LCCryptType type)
{
	...
	{
		case SIGNAL_NONE:
			Crypt_Set_Enable_SIP(0);
			Crypt_Set_Enable_RTP(0);
			break;
		case SIGNAL_SIP:
			Crypt_Set_Enable_SIP(1);
			Crypt_Set_Enable_RTP(0);
			break;
		case SIGNAL_RTP:
			Crypt_Set_Enable_SIP(0);
			Crypt_Set_Enable_RTP(1);
			break;
		default:
			break;
	}
...
LCCryptType linphone_core_get_transports_crypt(LinphoneCore *lc)
{
	 return lc->net_conf.crypt;
}

LCCryptSignal linphone_core_get_transports_crypt_signal(LinphoneCore *lc)
{
	return lc->net_conf.crypt_signal;

}

void linphone_core_set_transports_crypt_key(LinphoneCore *lc, const char *key)
{
	if(lc->net_conf.crypt_key!=NULL) ms_free(lc->net_conf.crypt_key);

	lc->net_conf.crypt_key=ms_strdup(key);
#ifdef ENABLE_MYCRYPT
	Crypt_Set_RC4_Key(lc->net_conf.crypt_key);
#endif // ENABLE_MYCRYPT
}

const char *linphone_core_get_transports_crypt_key(LinphoneCore *lc)
{
	return lc->net_conf.crypt_key;
}
